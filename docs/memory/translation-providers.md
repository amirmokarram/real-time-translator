...
**Settings & catalog centralization (added 2026-06-26):** `AppSettings`/`ProviderSettings` are now defined once in `shared/app-settings.d.ts` (a type-only `.d.ts` shared across the renderer/electron `rootDir` split; re-exported by `settings-store.ts` and `app.models.ts` — fixed a latent drift where the renderer copy lacked `ProviderSettings.prompt`). Default settings moved to `electron/config/default-settings.json`, imported with compile-time validation against `AppSettings`. The language catalog's single source of truth is now `src/app/core/models/languages.json`: the renderer imports it, and the build (`scripts/copy-electron-config.mjs`, wired into `electron:compile`/`electron:build`) copies it to `dist-electron/config/` for `electron/languages.ts` to read at runtime — the old duplicate hard-coded main catalog is gone. The unified `fa` display name is **`Persian`** (UI + export, unchanged); the prior LLM-prompt-only `Persian (Farsi)` divergence was dropped.
